...
def test_import_rss_url(import_rss, feed_repo, user):
...
def test_has_user_feeds(import_rss, feed_repo, user):
    user_feeds = feed_repo.get_user_feeds(user)
    assert len(user_feeds) == 1


@pytest.mark.django_db()
def test_has_user_entries(import_rss, feed_repo, user):
    user_feeds = feed_repo.get_user_feeds(user)
    user_feed = user_feeds[0]
...
@pytest.mark.django_db()
def test_entry_link(import_rss, feed_repo, user):
    user_feeds = feed_repo.get_user_feeds(user)
    user_feed = user_feeds[0]
    user_entries = feed_repo.get_filtered_user_entries(user_feed)
