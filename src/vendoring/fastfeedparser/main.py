...
def _clean_feed_content(content: str | bytes) -> str:
    ...
    if (
        content_lower.strip().startswith("<!doctype html")
        or content_lower.strip().startswith("<html")
        or "<script>" in content_lower
...
def parse(source: str | bytes, etag=None, modified=None) -> FastFeedParserDict:
    ...
    if (
        ...
        if channel is None:
            ...
            len(list(channel)) == 0
            and len([child for child in root if child.tag == "item"]) > 0
        ):
...
def _field_value_getter(
    ...
    if feed_type == "rss":
        ...
        def wrapper(
            ...
            if is_attr:
                result = _get_element_value(
                    root, atom_css, attribute="href"
                ) or _get_element_value(root, atom_css, attribute="link")
            else:
                result = _get_element_value(root, atom_css) or _get_element_value(
                    root, rdf_css
                )
...
custom_tzinfos: dict[str, int] = {
    ...
    "CST": 8 * 3600,  # noqa # China Standard Time
