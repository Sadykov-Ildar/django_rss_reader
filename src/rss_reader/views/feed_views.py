...
class FeedView(View):
    def post(self, request, *args, **kwargs):
        ...
        error_message = process_rss_url(request, rss_url, network_repo, rss_parser)
        if error_message:
    ...
    def delete(self, request, user_feed_id, *args, **kwargs):
        ...
        db_repo.delete_user_feed(user_feed)
...
def delete_all_user_feeds_view(request):
    db_repo.delete_user_feeds_for_user(request.user)
...
def export_user_feeds_view(request):
    feeds = db_repo.get_ordered_user_feeds(request.user).order_by("sort_order")
...
def mark_feeds_as_read_view(request):
    db_repo.mark_all_feeds_as_read(request.user)
...
def sort_user_feeds(request) -> HttpResponse:
    ...
    db_repo.reorder_user_feeds(user, user_feed_order)
