...
def import_from_rss_urls(user, rss_urls: list[str], network_repo) -> str:
    """
    ...
    for rss_url in rss_urls:
        try:
            created = db_repo.check_and_create_user_feed(rss_url, user)
            if not created:
    ...
    for request_result, parsed_data in parsed_results:
        ...
        if error_message:
            ...
            try:
                db_repo.create_feed_and_entries(user, parsed_data)
            except URLValidationError as e:
...
def process_rss_url(request, rss_url: str, network_repo, rss_parser):
    """
    ...
    if is_soup_html(soup):
        ...
        if rss_urls:
            error_message = import_from_rss_urls(user, rss_urls, network_repo)
            is_html = True
...
def refresh_feeds(network_repo) -> str:
    """
    ...
    feeds_by_urls = {}
    rss_urls_args = []
    feeds = db_repo.get_feeds_for_refresh()
    ...
    for request_result, parsed_data in parsed_results:
        ...
        db_repo.refresh_feed(feed, parsed_data, request_result)
